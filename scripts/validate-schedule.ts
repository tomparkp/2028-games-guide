...
import { readAllSessions } from './lib/content-store'
...
function groupBySession(rows: CsvRow[]): Map<string, CsvSession> {
  ...
  for (const r of rows) {
    ...
    if (existing) {
      ...
      if (!existing.sessionTypes.includes(r.sessionType)) existing.sessionTypes.push(r.sessionType)
    } else {
...
const sessions: Session[] = readAllSessions() as Session[]
