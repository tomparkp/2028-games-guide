...
const VENUE_SCORES: Record<string, number> = {
  'LA Memorial Coliseum': 10,
  'Rose Bowl Stadium': 10,
  'Rose Bowl Aquatics Center': 8,
  'Dodger Stadium': 10,
  '2028 Stadium': 9,
  'Intuit Dome': 8,
  'Alamitos Beach Stadium': 8,
  'Long Beach Arena': 6,
  'Carson Stadium': 6,
  'Carson Field': 5,
  'Exposition Park Stadium': 7,
  'DTLA Arena': 6,
  'Riviera Country Club': 8,
  'Fairgrounds Cricket Stadium': 6,
  'Galen Center': 6,
  'Venice Beach': 8,
  'Trestles State Beach': 8,
  'Long Beach Aquatics Center': 6,
  'Long Beach Climbing Theater': 6,
  'Belmont Shore': 7,
  'Peacock Theater': 6,
  'LA Convention Center Hall 2': 5,
  'Comcast Squash Center': 5,
  'Industry Hills MTB Course': 5,
  'Carson Velodrome': 6,
  'Whittier Narrows Clay Center': 5,
  'Long Beach Target Shooting Hall': 5,
  'Valley Complex 1': 5,
  'Valley Complex 2': 5,
  'Valley Complex 3': 5,
  'Valley Complex 4': 5,
...
export function rateEvent(event: {
  id: string
  desc: string
  venue: string
  time: string
  ...
  const isMeaningfulRound = ['gold_medal', 'final', 'finals_mixed', 'bronze', 'semifinal'].includes(
    roundType,
  )
  ...
  if (code === 'BKB' && ['gold_medal', 'final', 'semifinal'].includes(roundType))
    demand = clamp(demand + 1)
  if (
    code === 'BSB' &&
    event.venue.includes('Dodger Stadium') &&
    ['gold_medal', 'final'].includes(roundType)
  )
    demand = 10
